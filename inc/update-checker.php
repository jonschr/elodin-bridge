...
function elodin_bridge_set_update_checker_notice( $message ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		error_log( '[Elodin Bridge] ' . $message );
	}
}
...
function elodin_bridge_boot_update_checker() {
	...
	if ( '' === $repository || ! in_array( $repository, $allowed_repositories, true ) ) {
		elodin_bridge_set_update_checker_notice( 'Elodin Bridge update checker is disabled because the repository source is not in the approved allowlist.' );
		return;
	}
	...
	if ( null === $branch ) {
		elodin_bridge_set_update_checker_notice( 'Elodin Bridge update checker is disabled because the configured update branch is not allowlisted.' );
		return;
	}
